      ...
      <div className="max-w-7xl mx-auto">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 gap-4">
          ...
          <div className="flex gap-3">
            <button onClick={generarPDF} className="bg-black text-white px-4 py-2 rounded-lg hover:bg-gray-800 transition-colors flex items-center gap-2">Exportar PDF</button>
            <button onClick={() => router.push('/registrar')} className="bg-white border border-gray-300 text-gray-700 px-4 py-2 rounded-lg hover:bg-gray-50 transition-colors">Ir a Registro</button>
          </div>
        </div>
        ...
        <div className="bg-white p-4 md:p-6 rounded-xl shadow-sm border border-gray-100 mb-6 space-y-4">
          <div className="flex flex-col md:flex-row gap-4">
            <div className="flex-1">
              ...
              <select value={filtroUsuario} onChange={(e) => setFiltroUsuario(e.target.value)} className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-black outline-none text-amber-500 bg-white">
          ...
          <div className="flex flex-col md:flex-row gap-4 pt-4 border-t border-gray-100 items-end">
            <div className="flex-1 w-full">
              ...
              <input type="date" value={fechaInicio} onChange={(e) => setFechaInicio(e.target.value)} className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 text-blue-800 focus:ring-black outline-none" />
            </div>
            <div className="flex-1 w-full">
              <label className="block text-sm font-medium text-gray-700 mb-1">Hasta (Fecha Fin)</label>
              <input type="date" value={fechaFin} onChange={(e) => setFechaFin(e.target.value)} className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 text-blue-800 focus:ring-black outline-none" />
            </div>
        ...
        <h3 className="font-semibold text-gray-900 mb-3 flex items-center justify-between">
          ...
          <span className="text-sm font-normal text-gray-500">{carrerasFiltradas.length} carreras mostradas <span className="hidden md:inline">(Haz clic en un viaje para editarlo)</span></span>
        </h3>
              ...
              <table className="w-full text-sm text-left">
                ...
                <tbody className="divide-y divide-gray-100">
                  {carrerasFiltradas.map((carrera) => {
                    ...
                    return (
                      // AQUÍ ESTÁ LA MEJORA: Toda la fila tiene onClick, cambia el cursor y se ilumina al pasar el mouse
                      <tr 
                        key={carrera.id} 
                        onClick={() => setCarreraEditando(carrera)}
                        className="hover:bg-blue-50 cursor-pointer transition-colors group"
                        title="Haz clic para editar esta carrera"
                      >
